import { PacketNodeEncoded } from './packet-node-encoded';
import { PacketIconvEncoded } from './packet-node-iconv';
 ...
 */
export class PacketInputStream {
  unexpectedPacket: any;
  opts
  receiveQueue
  info
  out
  ...
  receivePacketDebug(packet) {
    ...
    if (packet) {
      ...
      if (this.opts.logPackets) {
        this.info.addPacket(
          `<== conn:${this.info.threadId ? this.info.threadId : -1} ${cmd
            ? cmd.onPacketReceive
              ? `${cmd.constructor.name}.${cmd.onPacketReceive.name}`
              : cmd.constructor.name
            : 'no command'} (${packet.pos},${packet.end}))
${packetStr}`
        );
      }
      if (this.opts.debug) {
