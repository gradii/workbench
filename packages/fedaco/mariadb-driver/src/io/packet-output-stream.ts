 ...
 * @constructor
 */
export class PacketOutputStream {
  opts;
  info;
  pos;
  buf;
  ...
  flushBufferDebug(commandEnd, remainingLen) {
    ...
    const packet = log(this.opts, this.buf, 0, this.pos);
    ...
    if (this.opts.debug) {
      console.log(
        ...
        log(this.opts, this.buf, 0, this.pos)
      );
    }
  ...
  writeEmptyPacket() {
    ...
    if (this.opts.logPackets || this.opts.debug) {
      const packet = log(this.opts, emptyBuf, 0, 4);
      if (this.opts.logPackets) {
        this.info.addPacket(
          '==> conn:' +
          (this.info.threadId ? this.info.threadId : -1) +
