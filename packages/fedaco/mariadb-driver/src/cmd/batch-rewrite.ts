...
import { CommonText } from './common-text-cmd';
import { Errors } from '../misc/errors';
import { splitRewritableNamedParameterQuery, splitRewritableQuery } from '../misc/parse';
import { RewritePacket } from '../io/rewrite-packet';
 ...
 */
export class BatchRewrite extends CommonText {
  ...
  start(out, opts, info) {
    ...
    if (this.opts.namedPlaceholders) {
      this.parseResults = splitRewritableNamedParameterQuery(this.sql, this.initialValues);
      this.values = this.parseResults.values;
    } else {
      this.parseResults = splitRewritableQuery(this.sql);
