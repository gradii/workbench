...
import { Ed25519PasswordAuth } from './handshake/auth/ed25519-password-auth';
import { NativePasswordAuth } from './handshake/auth/native-password-auth';
 ...
 */
export class ChangeUser extends Handshake {
  ...
  configAssign(connOpts, opt) {
    ...
    if (opt.charset && typeof opt.charset === 'string') {
      this.opts.collation = Collation.fromCharset(opt.charset.toLowerCase());
      if (this.opts.collation === undefined) {
        this.opts.collation = Collation.fromName(opt.charset.toUpperCase());
        if (this.opts.collation !== undefined) {
          console.log(
      ...
      this.opts.collation = Collation.fromName(initial.toUpperCase());
      if (this.opts.collation === undefined)
        throw new RangeError("Unknown collation '" + initial + "'");
    } else {
      this.opts.collation = Collation.fromIndex(opt.charsetNumber) || connOpts.collation;
    }
