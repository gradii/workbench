...
import Net from 'net';
import { PacketInputStream } from './io/packet-input-stream';
import { PacketOutputStream } from './io/packet-output-stream';
import { CompressionInputStream } from './io/compression-input-stream';
import { CompressionOutputStream } from './io/compression-output-stream';
import { ServerStatus } from './const/server-status';
import { ConnectionInformation } from './misc/connection-information';
import tls from 'tls';
import { Errors } from './misc/errors';
import { Utils } from './misc/utils';
import { Capabilities } from './const/capabilities';
import moment from 'moment-timezone';
import { Handshake } from './cmd/handshake/handshake';
import { Quit } from './cmd/quit';
import { Ping } from './cmd/ping';
import { Reset } from './cmd/reset';
import { Query } from './cmd/query';
import { BatchRewrite } from './cmd/batch-rewrite';
import { BatchBulk } from './cmd/batch-bulk';
import { Stream } from './cmd/stream';
import { ChangeUser } from './cmd/change-user';
import { ConnectionStatus } from './const/connection-status';
...
function Connection(options) {
    ...
    return new Promise(function(resolve, reject) {
      if (timeout) {
        ...
        const tOut = setTimeout(() => {
          ...
          if (_status !== ConnectionStatus.CLOSING && _status !== ConnectionStatus.CLOSED) {
            _sendQueue.clear();
  ...
  const _authSucceedHandler = (resolve, rejected) => {
    ...
    _status = ConnectionStatus.INIT_CMD;
  ...
  const _unexpectedPacket = function(packet) {
    if (packet && packet.peek() === 0xff) {
      ...
      if (err.fatal && _status !== ConnectionStatus.CLOSING && _status !== ConnectionStatus.CLOSED) {
        this.emit('error', err);
        this.end();
      }
    } else if (_status !== ConnectionStatus.CLOSING && _status !== ConnectionStatus.CLOSED) {
      this.emit(
        'error',
        Errors.createError(
  ...
  const _addCommandEnable = (cmd) => {
    ...
    if (_sendQueue.isEmpty() && (_status === ConnectionStatus.INIT_CMD || _status === ConnectionStatus.CONNECTED)) {
  ...
  const _socketErrorHandler = function(authFailHandler, err) {
    if (_status === ConnectionStatus.CLOSING || _status === ConnectionStatus.CLOSED) return;
    if (_socket) {
      _socket.writeBuf = () => {
      };
      _socket.flush = () => {
  ...
  const _fatalErrorHandler = function(self) {
    return function(err, avoidThrowError) {
      ...
      const mustThrowError = _status !== ConnectionStatus.CONNECTING;
      _status = ConnectionStatus.CLOSING;
  ...
  let _status = ConnectionStatus.NOT_CONNECTED;
