 ...
 */
export function splitQueryPlaceholder(sql, info, initialValues, displaySql) {
  let partList = [];
  let values = [];
  let state = State.Normal;
  let lastChar = '\0';
 ...
 */
export function splitRewritableQuery(sql) {
...
export function searchPlaceholder(sql, info, initialValues, displaySql) {
  let sqlPlaceHolder = '';
 ...
 * @returns {JSON} query separated by parameters
 */
export function splitRewritableNamedParameterQuery(sql, initialValues) {
  let reWritablePrepare = true;
  let multipleQueriesPrepare = true;
  let partList = [];
