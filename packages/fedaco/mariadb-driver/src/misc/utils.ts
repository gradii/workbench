...
import { CommonText } from '../cmd/common-text-cmd';
 ...
 */
export function log(opts, buf, off, end, header) {
  let out = [];
  if (!buf) return '';
...
export function escapeId(opts, info, value) {
  if (!value || value === '') {
...
export function escape(opts, info, value) {
