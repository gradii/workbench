...
export class ViewDefinitionsService {
  ...
  private renderDefinition(definition: Definition): ComponentRef<any> {
    const definitionFactory = this.componentFactoryResolver.resolveComponentFactory(definition.definition);
    const definitionRef = this.host.createComponent(definitionFactory);
