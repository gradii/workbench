...
import { OvenActions, Workflow } from '@common';
...
@Directive({ selector: '[ovenWithActions]' })
export class WithActionsDirective implements OnDestroy {
  @Input() set ovenWithActions(actions: OvenActions) {
    if (actions) {
  ...
  private listenActions(actions: OvenActions) {
