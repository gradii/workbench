...
export class ResizeMediator implements OnDestroy {
  ...
  private resize(rect: Rect) {
    ...
    const height: SpaceHeight = rect.dimension === 'horizontal' ? h : this.sizeMapper.createSpaceHeight(rect.height);
  ...
  private handleResizeAltStickNotification(): void {
    ...
    const showAltStick$ = resizing$.pipe(
      ...
      map(([_resizing, userNotifications]: [boolean, OvenUserNotifications]) => {
