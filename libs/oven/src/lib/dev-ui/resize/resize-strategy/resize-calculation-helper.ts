...
import { SpaceHeightType, SpaceWidthType, StylesCompilerService } from '@common';
...
export interface ResizeCalculationHelper {
  ...
  stickHeight(collision: CollisionBag, virtualComponent: VirtualComponent, initialCursorPosition: Point): boolean;
...
export class BaseResizeCalculationHelper implements ResizeCalculationHelper {
  ...
  autoStickAvailable(): Observable<boolean> {
    return this.attachedComponents$.pipe(
      take(1),
      map((attachedComponents: VirtualComponent[]) => {
  ...
  private cursorBelowThanParent(cursorPosition: Point, virtualComponent: VirtualComponent): boolean {
