...
import { SpaceWidthType, StylesCompilerService } from '@common';
...
export class CollisionDetector {
  ...
  private match(point: Point, virtualComponent: VirtualComponent, cursor: Point): CollisionBag {
  ...
  private activeComponent(component: VirtualComponent): boolean {
    return component.component.id === this.vc.component.id;
  }

  private collide(point: Point, virtualComponent: VirtualComponent, cursor: Point): CollisionBag {
  ...
  private cursorHorizontallyOutParent(cursor: Point) {
    const parentVirtualComponent = getParentVirtualComponent(this.vc);
    const parentRect = getAsBoundingClientRect(parentVirtualComponent);
    const paddingGap = this.getHorizontalPaddingGap(parentVirtualComponent);
  ...
  private getXCollision(point: Point, virtualComponent: VirtualComponent, currentMouse: Point): Collision {
  ...
  private getYCollision(point: Point, virtualComponent: VirtualComponent): Collision {
  ...
  private getXCollisionWithParent(point: Point, virtualComponent: VirtualComponent, currentMouse: Point): Collision {
    const { left, width } = getAsBoundingClientRect(virtualComponent);
    const leftCoordinate = Math.floor(left + this.window.scrollX);
  ...
  private getYCollisionWithParent(point: Point, virtualComponent: VirtualComponent): Collision {
    const { top, height } = getAsBoundingClientRect(virtualComponent);
    const topCoordinate = Math.floor(top + this.window.scrollY);
   ...
   * */
  private child(virtualComponent: VirtualComponent): boolean {
  ...
  private inViewport(virtualComponent: VirtualComponent): boolean {
  ...
  private getHorizontalPaddingGap(virtualComponent: VirtualComponent): number {
  ...
  private getVerticalPaddingGap(virtualComponent: VirtualComponent): number {
    const { paddings } = this.stylesCompiler.compileStyles(virtualComponent.component.styles);
    if (!paddings) {
      return 0;
    }
