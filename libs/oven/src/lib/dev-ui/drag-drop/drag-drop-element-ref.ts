...
export class DragDropElementRef implements DevUIElementRef {
  ...
  private projectComponents: Map<HTMLElement, VirtualComponent>;
  ...
  private createDragStartStream(event: MouseEvent): Observable<void> {
    ...
    return this.element$.pipe(
      ...
      tap(([[el, e], attachedComponents]: [[HTMLElement, MouseEvent], VirtualComponent[]]) => {
  ...
  private initProjectComponentsMap(attachedComponents: VirtualComponent[]) {
  ...
  private needToDecIndex(vc1: VirtualComponent, vc2: VirtualComponent, index: number): boolean {
  ...
  private moveIntoSpace(targetSpace: VirtualComponent) {
  ...
  private moveNearComponent(targetComponent: VirtualComponent, index: number) {
  ...
  private movingToSamePlaceIntoSpace(targetSpace: VirtualComponent): boolean {
    return this.virtualComponent.parentSlot.id === targetSpace.component.slots['content'].id;
  }

  private movingToSamePlaceNearComponent(targetComponent: VirtualComponent, index: number): boolean {
  ...
  private movingToSameSpace(space: VirtualComponent): boolean {
    if (isSpace(space)) {
