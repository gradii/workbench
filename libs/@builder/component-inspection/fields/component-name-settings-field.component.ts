...
import { COMMON_VARIABLE_NAME_PATTERN } from '@common/public-api';

import {
  AngularComponentValidator, UniqueComponentNameValidator, UniquePageNameValidator
  ...
  template       : `
    <form class="name-settings" [formGroup]="containerForm">
      ...
      <div *ngIf="canBeContainer" class="checkbox-container">
        <tri-checkbox formControlName="container">
          <span class="checkbox-label">Angular component</span>
        </tri-checkbox>
        <tri-icon
          triTooltip="Choose to mark element as separate Angular Component"
          triTooltipPosition="bottom"
          triTooltipClass="component-name-settings-tooltip"
...
export class ComponentNameSettingsFieldComponent implements OnInit, OnDestroy {
  ...
  private disableContainerCheckbox(actions: PuffActions) {
    if (!this.canBeContainer) {
      return;
    }
