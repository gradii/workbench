...
@Component({
  selector : 'pf-assign-variable-node',
  template : `
    <div (cdkObserveContent)="_onContentChanges()">
      ...
      <div class="description">
        {{node?.description}}
        <div class="edit-area" style="margin: 1rem;">
          <tri-select (valueChange)="onSelectionChange($event)">
            <tri-option value="v1">scope variable</tri-option>
          </tri-select>
        </div>
      </div>
      <div class="ports">
  ...
  styleUrls: ['./assign-variable-node.component.scss']
})
export class AssignVariableNodeComponent implements OnInit {
