...
import { ProjectProperties } from '../models/kitchen.models';
...
export class AnalyticsService {
  ...
  constructor(
    @Optional() private authService: TriAuthService,
  ...
  logCreateNewProject(templateName: string, projectType, props: ProjectProperties, error: string = '', source = 'dashboard'): void {
  ...
  logDuplicateProject(project: ProjectBrief): void {
    this.log('NEW_PROJECT', { creationType: 'duplicate', type: project.projectType, description: project.description });
  }
  ...
  logAddFeature(featureName: string): void {
    this.log('ADD_FEATURE', { featureName });
  }
  ...
  private listenAuthorizationStatus(): void {
    if (this.authService) {
      this.authService
        ...
        .subscribe((token: TriAuthOAuth2JWTToken) => this.logUserProperties(token));
    }
  }

  private logUserProperties(token: TriAuthOAuth2JWTToken): void {
