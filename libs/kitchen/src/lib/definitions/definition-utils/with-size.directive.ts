...
import { BoxSide, ComponentMargins, ComponentSize } from '@common/public-api';

@Directive({ selector: '[kitchenWithSize]' })
export class WithSizeDirective {
  private size: ComponentSize;
  private margins: ComponentMargins = {};

  @Input() set kitchenWithSize(size: ComponentSize) {
    if (size) {
      this.size = size;
    }
  }

  @Input() set kitchenWithSizeMargins(margins: ComponentMargins) {
    if (margins) {
