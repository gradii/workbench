...
export function extractDimensions(el: HTMLElement): ElementPadding {
  ...
  if (el.tagName.toLowerCase() === 'tri-tab-group') {
...
class PaddingElementRef implements DevUIElementRef {
  ...
  private show() {
    ...
    this.componentRef = this.ref.attach(new ComponentPortal(PaddingComponent));
...
export class PaddingService {
  ...
  createDevUI(virtualComponent: FlourComponent): DevUIElementRef {
