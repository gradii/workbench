...
import { ExtendedShadedColor, ShadedColor, Theme, ThemeColors, ThemeFont } from '@common/public-api';
...
export class ThemeService {
  ...
  updateRadius(radius: { value: number; unit: string }): Observable<Partial<Theme>> {
    return getActiveTheme.pipe(
      take(1),
      map((theme: Theme) => ({ id: theme.id, changes: { radius } }))
    );
  }

  updateShadow(shadowEnabled: boolean): Observable<Partial<Theme>> {
    return getActiveTheme.pipe(
      take(1),
      map((theme: Theme) => ({
        id     : theme.id,
  ...
  updateFont(font: ThemeFont): Observable<Partial<Theme>> {
    return getActiveTheme.pipe(
      take(1),
  ...
  lockColor(name: string, locked: boolean): Observable<Partial<Theme>> {
    return getActiveTheme.pipe(
      take(1),
      map((theme: Theme) => ({
        id     : theme.id,
  ...
  updatePrimary(colorChange: ColorChange): Observable<Partial<Theme>> {
  ...
  refreshSupport(): Observable<Partial<Theme>> {
    return getActiveTheme.pipe(
      take(1),
  ...
  updateShadedColor(name: string, value: ColorChange, count: number): Observable<Partial<Theme>> {
    return combineLatest([
