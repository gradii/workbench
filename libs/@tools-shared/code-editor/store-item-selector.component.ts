...
import { ActiveRouteId, dataNamespaces, getDeepValue, StoreItemType } from '@common/public-api';
import { BehaviorSubject, combineLatest, Observable, Subject } from 'rxjs';
import { debounceTime, map, takeUntil } from 'rxjs/operators';
import { TriOption } from '@gradii/triangle/core';
...
@Component({
  ...
  template       : `
    <div class="search-container">
      <tri-icon class="search-icon" svgIcon="eva:search-outline"></tri-icon>
      <input (input)="onSearch(searchInput.value)"
             #searchInput class="search-input"
             triInput fullWidth type="text" />
    </div>

    <div [style.width.px]="width" size="tiny">
      <tri-optgroup
        ...
        <ng-template #iconTemplate>
          <tri-icon
            *ngIf="getOptionGroupIcon(optionGroup.itemSource)"
            [svgIcon]="getOptionGroupIcon(optionGroup.itemSource)"
          >
          </tri-icon>
        </ng-template>
        <tri-option
...
export class StoreItemSelectorComponent implements OnInit, OnDestroy {
  ...
  @ViewChildren(TriOption) optionComponents: QueryList<TriOption>;
