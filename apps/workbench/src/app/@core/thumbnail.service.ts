...
import { MessageAction } from '@common';
...
export class ThumbnailService {
  ...
  constructor(private communicationService: CommunicationService, @Inject(DOCUMENT) private document) {
  }
  ...
  attach() {
    ...
    this.communicationService.makeThumbnail$
      ...
      .subscribe((image: string) =>
        this.communicationService.sendMessageWithPayload(MessageAction.MAKE_THUMBNAIL_RESPONSE, image)
      );
  }
  ...
  makeThumbnail(): Observable<string> {
    ...
    return from(
      html2canvas(el, {
        ...
        onclone: document => {
          ...
          const spaceList: NodeListOf<HTMLElement> = document.querySelectorAll('div.space');
          spaceList.forEach(space => {
            space.style.border = 'none';
          })
          // do not render broken svg
          const svgList: NodeListOf<SVGElementTagNameMap["svg"]> = document.querySelectorAll('svg');
    ...
    ).pipe(map((canvas: HTMLCanvasElement) => canvas.toDataURL('image/jpeg', 0.8)));
  }
  ...
  private calculateThumbSize(el: HTMLElement): { width: number; height: number } {
