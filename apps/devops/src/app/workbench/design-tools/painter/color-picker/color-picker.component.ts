...
export class ColorPickerComponent implements OnInit {
  ...
  public onHexInput(value: string | null) {
    if (value === null) {
      this.updateColorPicker();
    } else {
      if (value && value[0] !== '#') {
      ...
      if (this.service.isValidHex(value)) {
        this.setColorFromString(value);
      }
    }
  }

  public onHueInput(value: string | number) {
    value = value || '0';
    this.hsva.h = +value / 360;
    this.updateColorPicker();
  }

  public onValueInput(value: string | number) {
    value = value || '0';
    this.hsva.v = +value / 100;
    this.updateColorPicker();
  }

  public onSaturationInput(value: string | number) {
    value = value || '0';
    this.hsva.s = +value / 100;
    this.updateColorPicker();
  }
