...
import { ExecuteSourceType, KeyboardService } from '@common';
...
import { ComponentTreeNode, ComponentTreeNodeVisibility, ComponentTreeRenderNode } from './components-tree.model';
...
export class TreeElementListComponent implements OnInit, OnDestroy {
  ...
  private activeComponent$ = this.componentFacade.activeComponent$.pipe(
    filter((activeComponent: BakeryComponent) => !!activeComponent)
  );
  ...
  components$$: Observable<ComponentTreeRenderNode[]> = combineLatest([
    ...
    map(([components, invisibleComponents, collapsedComponents, breakpoint, activeComponent]) => {
  ...
  trackByFunction(index: number, item: ComponentTreeRenderNode): string {
    return item.component.ovenComponent.id;
  }

  selectComponent(component: ComponentTreeNode): void {
    this.componentFacade.selectComponentById(component.ovenComponent, ExecuteSourceType.ComponentTree);
  }

  private isActive(activeComponent: BakeryComponent, component: ComponentTreeNode): boolean {
    return activeComponent.id === component.ovenComponent.id;
  }
  ...
  private subscribeToCollapse(): void {
    ...
    combineLatest([this.activeComponent$, this.components$])
      .pipe(
        map(([active, components]: [BakeryComponent, ComponentTreeNode[]]) => {
          const collapsed: Set<number> = this.collapsedComponentsIndexes.getValue();
          const treeComponent = components.find(({ ovenComponent }) => active.id === ovenComponent.id);
          if (!treeComponent) {
            return collapsed;
          }
