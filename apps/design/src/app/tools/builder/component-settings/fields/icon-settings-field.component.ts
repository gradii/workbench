...
import { BcInputSearchOption, InputSearchDirective } from '@common';
import { NbIconLibraries, NbIconPack } from '@nebular/theme';
import { Subject } from 'rxjs';

@Component({
  selector: 'ub-icon-settings-field-action',
...
export class IconSettingsFieldComponent implements OnInit, OnDestroy {
  ...
  searchOptions: BcInputSearchOption[] = [];

  private destroyed$ = new Subject<void>();

  private pack: NbIconPack;

  constructor(nbIconLibraries: NbIconLibraries) {
    this.pack = nbIconLibraries.getPack('eva');
  }

  ngOnInit() {
    this.pack.icons.forEach((_, iconName: string) => {
      this.searchOptions.push({
        displayValue: iconName,
        filterValues: evaIconsTags[iconName],
        icon: iconName
      });
    });
  }
  ...
  update(icon: string) {
    if (this.pack.icons.has(icon)) {
      this.iconChange.emit(icon);
    }
  }

  select(option: BcInputSearchOption) {
    this.iconChange.emit(option.displayValue);
  }
}
