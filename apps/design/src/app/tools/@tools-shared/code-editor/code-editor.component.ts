...
import {
  ...
  // @ts-ignore function exist, error in typings
  multiplexingMode
} from 'codemirror';
...
@Component({
  ...
  template: `
    <div
      ...
      [nbPopover]="variableHint"
      nbPopoverPlacement="top"
      nbPopoverAdjustment="vertical"
      nbPopoverTrigger="noop"
      [nbPopoverClass]="['no-arrow', 'autocomplete-trigger-hint-popover']"
      [nbPopoverOffset]="4"
    >
...
export class CodeEditorComponent implements OnInit, OnChanges, AfterViewInit, OnDestroy, ControlValueAccessor {
  ...
  @ViewChild(NbPopoverDirective) autocompleteTriggerHintPopover: NbPopoverDirective;
  ...
  private defineSyntaxMode(modeName: string, baseSyntax: CodeEditorSyntax) {
    defineMode(modeName, config => {
      ...
      return multiplexingMode(baseMode, interpolationMode);
    });
  }
}
