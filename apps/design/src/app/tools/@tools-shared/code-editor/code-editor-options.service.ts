...
import { LocalStorageFacadeService } from '../../workflow/util/local-storage-facade.service';
...
export class CodeEditorOptionsService {
  ...
  resolveFromListOptionGroup(component: OvenComponent, ingnoreSelf: boolean): Observable<OptionGroup[]> {
    return this.componentFacade.findParentListComponent(component.id).pipe(
      map((sequenceComponents: BakeryComponent[]) => {
        if (!sequenceComponents) {
          return [];
        }

        if (ingnoreSelf) {
          sequenceComponents = sequenceComponents.filter((c: BakeryComponent) => c.id !== component.id);
        }

        return sequenceComponents
          .map((c: BakeryComponent) => {
